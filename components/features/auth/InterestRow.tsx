...
export default function InterestRow({ value, onChange }: InterestRowProps) {
  ...
  return (
    <div className="flex gap-2">
      <div className="relative max-w-30 flex-1">
        ...
        {openDropdown === 'major' && (
          <Dropdown items={majors} onSelect={handleSelectMajor} className="mt-2" />
        )}
      </div>
      ...
      <div className="relative max-w-78 flex-1">
        ...
        >
          <span className="font-medium text-sm">{value.minor || '상세 분야'}</span>
          {openDropdown === 'minor' ? (
            <ChevronUp width={16} height={16} color="#94a3b8" />
          ) : (
            <ChevronDown width={16} height={16} color="#94a3b8" />
        ...
        {openDropdown === 'minor' && (
          <Dropdown items={minors} onSelect={handleSelectMinor} className="mt-2" />
        )}
      </div>
    </div>
  );
